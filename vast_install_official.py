    ...
    if __name__ == "__main__":
        ...
        if not args.no_daemon:
            ...
            if not args.amd:
                ...
                if (num_gpus > 1):
                    log(f"\033[37;1m    Testing {num_gpus} gpus for NCCL errors (30 seconds)", level=0)
                    output = process_check_output(f"docker run --shm-size 4G --rm --runtime=nvidia --gpus all pytorch/pytorch bash -c 'apt update; apt-get install -y wget; wget https://s3.amazonaws.com/vast.ai/test_NCCL.py; python3 test_NCCL.py --num-gpus {num_gpus} --backend NCCL;'", shell=True)
